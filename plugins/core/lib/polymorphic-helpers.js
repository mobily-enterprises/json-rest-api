...
/**
 * Creates polymorphic helper functions with injected dependencies
 * 
 ...
 * @returns {Object} Object containing all polymorphic helper functions
 * 
 * @example
 ...
 * 
 * // Validate a polymorphic relationship configuration
 * const validation = polymorphicHelpers.validatePolymorphicRelationship(relDef, 'comments');
 * if (!validation.valid) {
...
export const createPolymorphicHelpers = (scopes, log) => {
   ...
   * };
   * 
   * const result = validatePolymorphicRelationship(relDef, 'comments');
   * // Returns: { valid: true }
   * 
   ...
   * };
   * 
   * const result = resolvePolymorphicTarget(comment, 'commentable_type', 'commentable_id', ['articles']);
   ...
   * };
   * 
   * const aliasMap = await buildPolymorphicSearchJoins(query, searchDef, 'activities', 'activities', knex);
   * 
   * // Returns: {
