...
export const AlaSQLAdapter = {
  install(api, options = {}) {
    ...
    // Implement atomic shift positions for concurrent safety
    api.implement('atomicShiftPositions', async (context) => {
      ...
      // Add exclude IDs condition
      if (excludeIds.length > 0) {
        const placeholders = excludeIds.map(() => '?').join(',');
        conditions.push(`\`${api.options.idProperty || 'id'}\` NOT IN (${placeholders})`);
      ...
      const sql = `UPDATE \`${type}\` SET \`${field}\` = \`${field}\` + ? ${whereClause}`;
      const result = await api.execute('db.query', { sql, params: [delta, ...values] });
      
      return { shifted: result.info?.affectedRows || 0 };
    });
    ...
    api.storagePlugin.atomicShiftPositions = async (type, params) => {
      return await api.execute('atomicShiftPositions', { type, params });
    };
  }
};