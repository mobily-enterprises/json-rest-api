...
export class Schema {
  constructor(structure = {}, options = {}) {
    ...
    this.options = {
      strictMode: options.strictMode !== false, // Enable strict type validation by default
      maxItems: options.maxItems || 1000,     // Maximum array size
      maxKeys: options.maxKeys || 100,        // Maximum object keys
      maxDepth: options.maxDepth || 10,       // Maximum nesting depth
      ...options
    };
    this.plugins = [];
    this.types = new Map();
  ...
  _initializeBuiltins() {
    ...
    this.registerType('datetime', this._dateTimeType); // Alias for backward compatibility
  ...
  async validate(object, options = {}) {
    ...
    if (!options.onlyObjectValues) {
      for (const fieldName in object) {
        ...
        } else if (this.structure[fieldName].virtual) {
          // Remove virtual fields from the object - they shouldn't be stored
          delete validatedObject[fieldName];
        }
      }
    }
    ...
    for (const fieldName in targetObject) {
      ...
      // Skip virtual fields - they're computed, not stored
  ...
  _numberType({ value, definition }) {
    if (value === undefined) return 0;
    
    // In strict mode, only accept actual numbers
    const strictMode = definition.strictNumber !== undefined ? definition.strictNumber : this.options.strictMode;
    if (strictMode && typeof value !== 'number' && value !== null) {
  ...
  _arrayType({ value, definition }) {
    // Use global strict mode if not explicitly set on field
    const strictMode = definition.strictArray !== undefined ? definition.strictArray : this.options.strictMode;
    
    if (strictMode) {
      // Only allow actual arrays, not auto-conversion
      if (!Array.isArray(value) && value !== undefined && value !== null) {
        throw new Error('Value must be an array');
      }
    }
    
    // Legacy behavior for backward compatibility when strict mode is false
    return Array.isArray(value) ? value : (value === undefined || value === null ? [] : [value]);
  }

  _objectType({ value, definition }) {
    // Use global strict mode if not explicitly set on field
    const strictMode = definition.strictObject !== undefined ? definition.strictObject : this.options.strictMode;
    
    if (strictMode) {
      // Only allow actual objects (not arrays, null, or primitives)
  ...
  _booleanType({ value, definition }) {
    // Use global strict mode if not explicitly set on field
    const strictMode = definition.strictBoolean !== undefined ? definition.strictBoolean : this.options.strictMode;
    
    if (strictMode) {
      // Only allow actual booleans, not string conversions
    ...
    if (typeof value === 'string') {
      ...
      if (value === falseValue || value === '0' || value === 'off' || value === 'no') return false;
      if (value === trueValue || value === '1' || value === 'on' || value === 'yes') return true;
      return false;
    }
    return !!value;
  }
  ...
  async _formatParam(context) {
    ...
    // Don't return anything - parameter handlers should only return a value if they want to modify it
  }
}