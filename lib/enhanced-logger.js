...
export function createEnhancedLogger (baseLogger, options = {}) {
    ...
    return args.map(arg => {
      try {
        if (arg instanceof Error) {
          return formatLoggedError(arg, diagnosticOptions)
        }

        /** @type {unknown} */
        let nestedError
        if (arg && typeof arg === 'object' && !diagnosticOptions.redactFields.includes('error')) {
          nestedError = Reflect.get(arg, 'error')
        }
        ...
        // The shared formatter can contain values that cannot even be classified.
        return arg
      }
  ...
  for (const level of logLevels) {
    if (typeof baseLogger[level] === 'function') {
      ...
      enhancedLogger[level] = function (...args) {
        const processedArgs = processArgs(args.slice(0, 50))
  ...
  const logValidationError = function (message, error, additionalData = {}) {
    ...
    if (Array.isArray(violations)) {
      ...
      return this.logError(message, error, additionalData)
    }
  }
