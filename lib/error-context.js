 ...
 * @returns {boolean}
 */
function hasPrototype (value, prototype) {
  if (value === null || (typeof value !== 'object' && typeof value !== 'function')) return false
  /** @type {object | null} */
  let current = value
  try {
    while (current !== null) {
      // Ordinary prototype chains cannot cycle; proxies can invent their next link.
      if (types.isProxy(current)) return false
      current = Object.getPrototypeOf(current)
      if (current === prototype) return true
    }
  } catch { /* Unreadable prototypes do not establish identity. */ }
  return false
}

/** @param {unknown} error @returns {error is Error} */
export function isErrorObject (error) {
  return types.isNativeError(error) || hasPrototype(error, Error.prototype)
}

/**
 * @param {unknown} error
 * @returns {error is RestApiError}
 */
export function isRestApiError (error) {
  return hasPrototype(error, RestApiError.prototype)
}
...
export function getBinaryDiagnostic (value) {
  if (ArrayBuffer.isView(value)) {
    const type = hasPrototype(value, Buffer.prototype) ? 'Buffer' : 'ArrayBufferView'
