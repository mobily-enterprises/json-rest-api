...
export class Api {
  ...
  async checkIncludePermission(user, fieldDef, record = null) {
    // Check if field has refs (is a relationship)
    if (!fieldDef.refs) return false;
    ...
    for (const fieldName in processedByPath) {
      ...
      if (this.options.jsonApiCompliant) {
        ...
        if (!context.included.has(includedKey)) {
          // Format the included resource with permission filtering
          const formatted = this._formatResource(processedData, joinMeta.resource, null, context);
          if (formatted) {
   ...
   */
  _formatResource(resource, type, fields = null, context = null) {
        ...
        for (const [field, nestedJoinConfig] of Object.entries(nestedConfig)) {
          ...
          try {
            ...
            if (nestedResult.data) {
              const formatted = this._formatResource(nestedResult.data, targetResource, null, context);
              if (formatted) {
