...
export class Api {
  ...
  async query(params = {}, options = {}) {
    ...
    if (params.fields && options.type) {
      ...
      const fieldsToValidate = params.fields[options.type] || params.fields;
      if (fieldsToValidate) {
  ...
  _formatResource(resource, type) {
    ...
    if (schema) {
      for (const [field, def] of Object.entries(schema.structure)) {
        if (def.type === 'id' && attributes[field] !== null && attributes[field] !== undefined) {
          // Convert ID fields to strings unless they're objects (joined data)
          if (typeof attributes[field] !== 'object') {
