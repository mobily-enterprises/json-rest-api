...
for (const mode of modes) {
  ...
  try {
    ...
    const publisher = await api.resources.publishers.post({ data: { name: 'Migration publisher' } })
    await api.resources.books.post({ data: { id: '42', title: 'Original' } })
    ...
    const writeBlocks = blocks('Replace inputRecord with data or document')
    ...
    assert.equal((await api.resources.books.get({ id: '42' })).title, 'Updated title')
    const other = await api.resources.books.post({ data: { title: 'Other', publisher: publisher.id } })
    ...
    await api.resources.books.patch({ id: '42', data: { title: 'Before managed change' } })
    ...
    await api.resources.books.patch({ id: '42', data: { title: 'Before failed unit' } })
