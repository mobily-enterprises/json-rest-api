...
try {
  ...
  const country = await countries.post({ inputRecord: { name: 'Australia' } })
  ...
  await countries.post({ inputRecord: { name: 'Australia' } })
  await drainSocketEvents(socket)
  assert.equal(changes.length, 4)
