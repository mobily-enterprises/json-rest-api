# Migrating to the simpler API

**Unreleased breaking API.** Upgrade each application's source and its
jskit-ai/json-rest-api dependencies together, after verifying the matching
...
`inputRecord.data.type`. User/workspace repositories in jskit-ai relied on that
behavior. A rename alone is insufficient for those calls. If the repository
...
existing domain normalization. The shared jskit-ai CRUD repository still uses
JSON:API documents intentionally and selects `format: 'jsonapi'`.
...
select its response contract explicitly; jskit-ai's JSON:API repositories do so.
...
The [Fastify guide](GUIDE_X_Fastify.md) documents body limits, prefixed 404s and
...
for an unknown outcome. See the [commit uncertainty contract](transaction-outcomes.md#when-commit-acknowledgement-is-lost)
...
read-only API operations. See the [managed transaction contract](managed-transactions.md).
...
completed. The [outcome contract](transaction-outcomes.md) explains
...
1. Confirm the library package resolves to the intended new local artifact, and
   each app resolves to the intended migrated jskit-ai packages.
2. Run library checks and the affected jskit-ai/app tests; an old installed
...
5. Recheck new call sites introduced by the concurrent jskit-ai expansion, and
   remove obsolete wrappers and imports once their replacements pass.

The exact paired verification commands and artifact versions will be recorded
with the implementation. Completion of this goal requires the jskit-ai/vibe64/seed
ports and this usable guide. Other app ports are separate owner-managed work;
apply the verification steps above within each app when updating it.

## TypeScript imports

The working package now exposes declarations through `json-rest-api` itself:
...
`npm run test:public-types` checks a packed consumer; actual app migration and
automatic API type integration remain unfinished.
...
isolated synthetic migrations. Consumer repositories and their seeds remain
on hold and have not been migrated by this batch. The broader schema/migration
...
resource queries. No imports of those helpers were found in jskit-ai, vibe64 or
its public/accounts seeds.
...
[AnyAPI field additions](GUIDE_X_Knex_Schema_And_Migrations.md#anyapi-field-additions).
...
[schema and migration guide](GUIDE_X_Knex_Schema_And_Migrations.md) for execution
...
can declare their own computed `label`; see the [label guide](GUIDE_X_Labels.md).
...
[dependency contract](GUIDE_3_Field_Transformations.md#dependency-contract)
...
See the [plugin example](GUIDE_X_Writing_Plugins.md#the-query-field-seam).
...
it. The scoped jskit-ai/vibe64/seed source inventory found no calls to these
table/migration helpers, so this batch requires no consumer data conversion.
...
migration paths are documented in the query-plan review (source checkout: `docs/development/query-plans.md`).
...
origins for credentialed applications. See the [CORS guide](GUIDE_X_Cors.md).
...
See the [file ownership contract](GUIDE_X_File_Uploads.md#ownership-after-commit).
...
[storage behavior](GUIDE_X_File_Uploads.md#storage-adapters).
...
Review the documented [bounded defaults](GUIDE_X_File_Uploads.md#protocol-configuration)
...
[file ownership after commit](GUIDE_X_File_Uploads.md#ownership-after-commit)
...
required. The [Socket.IO guide](GUIDE_X_SocketIO.md) lists the verified paths and
...
The [Socket.IO guide](GUIDE_X_SocketIO.md) separates verified local behavior from
...
file cleanup and notifications now use the managed owner in the working tree;
broader integration verification is unfinished. No stored-data migration is needed.

No in-scope jskit-ai, vibe64 or seed source uses the bulk plugin or its methods.
That statement comes from the captured consumer inventory; current consumer
verification is paused. Bulk argument consolidation, the managed transaction
contract and deferred events remain open in the master plan.
...
read. See the [row-policy contract](GUIDE_X_Row_Policies.md#query-purposes-and-lifecycle-coverage)
...
No option retains the old target GET lifecycle. Consumer migration is still
pending while jskit-ai/vibe64 work is paused.
...
and before storage, without completing the transaction. Current jskit-ai source
does not import this helper, so its resource calls need no change for this
internal simplification. The [lifecycle record](GUIDE_7_Hooks_Data_Management_And_Plugins.md)
...
[version-field migration guide](version-field-migration.md) for
...
See the concurrency design and acceptance record (source checkout: `docs/development/optimistic-concurrency.md`)
for the complete handling table, tested scope and remaining acceptance work.
