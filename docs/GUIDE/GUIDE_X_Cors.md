...
Use hooked-api's `beforeFunction`/`afterFunction` placement options when hook
order matters; a numeric `order` option is not supported by hooked-api.
...
needed; see the [Fastify guide](GUIDE_X_Fastify.md). Express route-parameter decode
