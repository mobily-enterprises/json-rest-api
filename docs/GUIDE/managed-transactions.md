# Managed transaction contract

**Core implementation verified; integration remains in progress.** The callback
helper and shared resource/relationship/bulk/registry ownership now pass selected
Node 24 checks in both storage modes on SQLite, PostgreSQL and MySQL. This closes
B2-03/B2-04/B2-06/B2-08; see evidence and scope (source checkout: `docs/development/verification-progress.md`).
Library transaction callers and examples are migrated, and the full Node 24 gate
now passes. B2-09's explicit file/notification integration checks also pass;
see the repeated-operation checks (source checkout: `docs/development/verification-progress.md`).
B2-05's completion ordering and finalizer failure audit also passes; see
ordering evidence (source checkout: `docs/development/verification-progress.md`).
The broader A7 failure audit, remaining public documentation and consumer
migration are unfinished. This is not a completed or released feature.
Consumer migration remains paused.
...
## Nesting and consumer migration
...
The captured consumer inventory identifies `createWithTransaction` delegating
to `runInTransaction`/Knex. When consumer work resumes, reconcile its current
implementation and make the repository entrypoint delegate to this helper, or
remove it if callers no longer need it. Port raw transaction creation, context
reuse and error handling together. Preserve useful domain scope resolution.
Do not keep both transaction engines or add a runtime old/new translator.

B2-01/B2-07/B2-10 remain open for the consumer ownership model,
remaining caller migration and the actual consumer workflow. B2-05/B2-09 close
the specified completion-ordering and managed upload/notification checks;
they do not complete the broader A7 guarantees or consumer requirements.
