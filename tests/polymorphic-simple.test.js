...
describe('Polymorphic Relationships - Basic Tests', () => {
  ...
  it('should create and retrieve a review for a book', async () => {
    ...
    assert.equal(retrievedReview.data.relationships.reviewable.data.type, 'books')
    assert.equal(retrievedReview.data.relationships.reviewable.data.id, String(book.data.id))
  })
  ...
  it('should query all reviews', async () => {
    ...
    // Polymorphic fields are no longer in attributes, they're in relationships
    const bookReview = result.data.find(r => r.relationships.reviewable.data.type === 'books')
    const authorReview = result.data.find(r => r.relationships.reviewable.data.type === 'authors')
