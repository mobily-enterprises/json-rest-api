...
describe('Unavailable include-window capability', () => {
  ...
  beforeEach(async () => {
    ...
    await fixture.reset()
    await fixture.api.resources.groups.post({ inputRecord: { data: { type: 'groups', id: '100', attributes: { name: 'Original' } } } })
    await fixture.api.resources.items.post({ inputRecord: { data: { type: 'items', id: '101', attributes: { name: 'Child' }, relationships: { group: { data: { type: 'groups', id: '100' } } } } } })
  ...
  for (const returning of ['none', 'minimal']) {
    it(`allows ${returning} writes that do not execute an included response`, async () => {
      const result = await fixture.api.resources.groups.patch({
        ...
        inputRecord: { data: { type: 'groups', id: '100', attributes: { name: 'Changed' } } }
      })
    ...
    it(`rejects ${method} without executing a window query or retaining partial changes`, async () => {
      ...
      try {
        await assert.rejects(fixture.api.resources.groups[method]({
          ...
          returning: 'full',
          queryParams: { include: ['items'] },
          ...(method === 'patch' ? { inputRecord: { data: { type: 'groups', id: '100', attributes: { name: 'Changed' } } } } : {})
