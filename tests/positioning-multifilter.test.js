...
describe('Multi-Filter Positioning Tests', { timeout: 30000 }, () => {
  ...
  describe('Multiple Filter Fields', () => {
    it('should maintain separate position sequences for each filter combination', async () => {
      ...
      const p1ActiveItems = await api.resources.items.query({
        queryParams: {
      ...
      const p1ArchivedItems = await api.resources.items.query({
        queryParams: {
      ...
      const p2ActiveItems = await api.resources.items.query({
        queryParams: {
    ...
    it('should handle positioning with null values in multi-filter setup', async () => {
      ...
      const projectActiveItems = await api.resources.items.query({
        queryParams: {
      ...
      const projectNullStatusItems = await api.resources.items.query({
        queryParams: {
      ...
      const nullProjectActiveItems = await api.resources.items.query({
        queryParams: {
      ...
      const bothNullItems = await api.resources.items.query({
        queryParams: {
    ...
    it('should handle beforeId correctly with multi-filter positioning', async () => {
      ...
      const archivedItems = await api.resources.items.query({
        queryParams: {
      ...
      const activeItems = await api.resources.items.query({
        queryParams: {
          filters: { project_id: project.id, status: 'active' },
          sort: ['position']
        }
    ...
    it('should handle updates that change filter values', async () => {
      ...
      const p1Items = await api.resources.items.query({
        queryParams: {
          filters: { project_id: project1.id, status: 'active' },
          sort: ['position']
        }
      ...
      const p2Items = await api.resources.items.query({
        queryParams: {
          filters: { project_id: project2.id, status: 'active' },
          sort: ['position']
        }
  ...
  describe('Complex Multi-Filter Scenarios', () => {
    it('should handle three or more filter fields', async () => {
      ...
      for (const status of statuses) {
        for (const priority of priorities) {
          const filtered = await multiApi.resources.items.query({
            queryParams: {
              filters: { 
                project_id: project.id, 
                status: status,
      ...
      const activeHighItems = await multiApi.resources.items.query({
        queryParams: {
          filters: { 
            project_id: project.id, 
            status: 'active',
    ...
    it('should handle filter changes during bulk operations', async () => {
      ...
      const p1Active = await api.resources.items.query({
        queryParams: {
          filters: { project_id: project1.id, status: 'active' },
          sort: ['position']
        }
      ...
      const p1Archived = await api.resources.items.query({
        queryParams: {
          filters: { project_id: project1.id, status: 'archived' },
          sort: ['position']
        }
      ...
      const p2Active = await api.resources.items.query({
        queryParams: {
          filters: { project_id: project2.id, status: 'active' },
          sort: ['position']
        }
      ...
      const p2Archived = await api.resources.items.query({
        queryParams: {
  ...
  describe('Edge Cases with Multi-Filter', () => {
    it('should handle empty string vs null in filters', async () => {
      ...
      const emptyStatusItems = await api.resources.items.query({
        queryParams: {
      ...
      const nullStatusItems = await api.resources.items.query({
        queryParams: {
          filters: { project_id: project.id, status: null },
          sort: ['position']
        }
    ...
    it('should handle numeric vs string filter values', async () => {
      ...
      // Query with string '1'
      const status1Items = await api.resources.items.query({
        queryParams: {
      ...
      const status01Items = await api.resources.items.query({
        queryParams: {
