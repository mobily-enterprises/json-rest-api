...
describe('Multi-Filter Positioning Tests', { timeout: 30000 }, () => {
  ...
  describe('Multiple Filter Fields', () => {
    it('should maintain separate position sequences for each filter combination', async () => {
      ...
      // Create projects
      const project1 = await api.resources.projects.post({ format: 'plain', inputRecord: { name: 'Project 1' } })
      const project2 = await api.resources.projects.post({ format: 'plain', inputRecord: { name: 'Project 2' } })

      // Create items in different filter combinations
      // Project 1, Active
      const p1Active1 = await api.resources.items.post({ format: 'plain', inputRecord: { name: 'P1 Active 1', project: project1.id, status: 'active' } })

      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'P1 Active 2', project: project1.id, status: 'active' } })

      // Project 1, Archived
      const p1Archived1 = await api.resources.items.post({ format: 'plain', inputRecord: { name: 'P1 Archived 1', project: project1.id, status: 'archived' } })

      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'P1 Archived 2', project: project1.id, status: 'archived' } })

      // Project 2, Active
      const p2Active1 = await api.resources.items.post({ format: 'plain', inputRecord: { name: 'P2 Active 1', project: project2.id, status: 'active' } })

      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'P2 Active 2', project: project2.id, status: 'active' } })

      // Project 2, Archived
      const p2Archived1 = await api.resources.items.post({ format: 'plain', inputRecord: { name: 'P2 Archived 1', project: project2.id, status: 'archived' } })
    ...
    it('should handle positioning with null values in multi-filter setup', async () => {
      ...
      const project = await api.resources.projects.post({ format: 'plain', inputRecord: { name: 'Project' } })

      // Create items with various null combinations
      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 1', project: project.id, status: 'active' } })

      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 2', project: project.id, status: null } })

      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 3', project: null, status: 'active' } })

      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 4', project: null, status: null } })
    ...
    it('should handle beforeId correctly with multi-filter positioning', async () => {
      ...
      const item2 = await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 2', project: project.id, status: 'active' } })

      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 3', project: project.id, status: 'active' } })

      // Create item in different filter group with beforeId from first group
      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 4', project: project.id, status: 'archived', beforeId: item2.id } })
      ...
      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 5', project: project.id, status: 'active', beforeId: item2.id } })
    ...
    it('should handle updates that change filter values', async () => {
      ...
      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 1', project: project1.id, status: 'active' } })

      const item2 = await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 2', project: project1.id, status: 'active' } })

      const item3 = await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 3', project: project1.id, status: 'active' } })

      // Create items in project 2
      const item4 = await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 4', project: project2.id, status: 'active' } })

      // Move item2 to project 2, positioning it before item4
      await api.resources.items.patch({ id: item2.id, format: 'plain', inputRecord: { project: project2.id, beforeId: item4.id } })
      ...
      await api.resources.items.patch({ id: item3.id, format: 'plain', inputRecord: { status: 'archived' } })
  ...
  describe('Complex Multi-Filter Scenarios', () => {
    it('should handle three or more filter fields', async () => {
      ...
      const project = await multiApi.resources.projects.post({ format: 'plain', inputRecord: { name: 'Project' } })
      ...
      for (const status of statuses) {
        for (const priority of priorities) {
          const item = await multiApi.resources.items.post({ format: 'plain', inputRecord: { name: `Item ${status}-${priority}`, project: project.id, status, priority } })
          items.push(item)
        }
      }
      ...
      const moreItems = []
      for (let i = 2; i <= 5; i++) {
        const item = await multiApi.resources.items.post({ format: 'plain', inputRecord: { name: `Item active-high ${i}`, project: project.id, status: 'active', priority: 'high' } })
        moreItems.push(item)
      }
    ...
    it('should handle partial filter queries correctly', async () => {
      ...
      // Create items
      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'P1 Active 1', project: project1.id, status: 'active' } })

      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'P1 Active 2', project: project1.id, status: 'active' } })

      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'P1 Archived', project: project1.id, status: 'archived' } })

      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'P2 Active', project: project2.id, status: 'active' } })
    ...
    it('should handle filter changes during bulk operations', async () => {
      ...
      const items = []
      for (let i = 1; i <= 10; i++) {
        const item = await api.resources.items.post({ format: 'plain', inputRecord: { name: `Item ${i}`, project: project1.id, status: 'active' } })
        items.push(item)
      }
      ...
      const updates = [
        // Move to archived
        api.resources.items.patch({ id: items[0].id, format: 'plain', inputRecord: { status: 'archived' } }),
        api.resources.items.patch({ id: items[1].id, format: 'plain', inputRecord: { status: 'archived' } }),
        api.resources.items.patch({ id: items[2].id, format: 'plain', inputRecord: { status: 'archived' } }),

        // Move to project 2
        api.resources.items.patch({ id: items[3].id, format: 'plain', inputRecord: { project: project2.id } }),
        api.resources.items.patch({ id: items[4].id, format: 'plain', inputRecord: { project: project2.id } }),

        // Move to project 2 and archived
        api.resources.items.patch({ id: items[5].id, format: 'plain', inputRecord: { project: project2.id, status: 'archived' } })
      ]
  ...
  describe('Edge Cases with Multi-Filter', () => {
    it('should handle empty string vs null in filters', async () => {
      ...
      const item1 = await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 1', project: project.id, status: '' } })

      const item2 = await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 2', project: project.id, status: null } })

      const item3 = await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 3', project: project.id, status: 'active' } })
    ...
    it('should handle numeric vs string filter values', async () => {
      ...
      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 1', project: project.id, status: '1' } })

      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 2', project: project.id, status: '01' } })

      await api.resources.items.post({ format: 'plain', inputRecord: { name: 'Item 3', project: project.id, status: '1' } })
